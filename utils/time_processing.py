...
def find_nearest_available_time(user_id: int, time: datetime, busy_slots: list):
    ...
    for busy_slot in busy_slots:
        ...
        logger.info(f"КІНЦЕВИЙ ЧАС: {busy_slot['end']}")

        if busy_slot["start"].time() <= current_time <= busy_slot["end"].time():
            logger.info(
                f"УВІЙШЛИ В УМОВНУ ПЕРЕВІРКУ. ВКАЗАНИЙ КОРИСТУВАЧЕМ ЧАС В ДІАПАЗАОНІ ЗАЙНЯТОГО ЧАС"
            )
...
def check_slot(user_id: int, time: datetime):
    ...
    if busy_slots:
        ...
        for slot in busy_slots:
            if slot["start"].time() <= time <= slot["end"].time():
...
def time_check(time: datetime, date: FreeDate):
    ...
    combine_datetime = datetime.combine(date.date, time)
